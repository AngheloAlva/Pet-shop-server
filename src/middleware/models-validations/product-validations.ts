...
export const updateProductValidation = [
  ...
  body('slug').optional().isString().isLength({ min: 3, max: 255 }),
  body('miniDesc').optional().isString().isLength({ min: 20, max: 255 }),
  body('description').optional().isArray(),
  body('description.*.title').optional().isString().isLength({ min: 3, max: 255 }),
  body('description.*.content').optional().isString(),
  body('images').optional().isArray().withMessage('Images must be an array'),
  body('brandId').optional().isNumeric().withMessage('Brand id must be a number'),
  body('categoryId').optional().isNumeric().withMessage('Category id must be a number'),
  body('images.*').optional().isString().withMessage('Images must be an array of strings'),
  body('petType').optional().isIn(petTypes).withMessage('Pet type must be one of: DOG, CAT, BIRD, FISH, REPTILE, SMALL_ANIMAL'),
  body('lifeStage').optional().isIn(lifeStages).withMessage('Life stage must be one of: PUPPY, ADULT, SENIOR, KITTEN'),
  validate
]
